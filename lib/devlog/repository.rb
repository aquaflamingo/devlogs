...
class Repository
  ...
  DEFAULT_TIME_FORMAT_FILE_PREFIX = "%m-%d-%Y_%k%M"
    ...
    if @config.mirror?
      ...
      Rsync.run('-av', path, @config.mirror.path) do |result|
        if result.success?
          ...
          raise RuntimeError.new("Failed to sync: #{result.error}")
        end
      end
    end
  end    
  ...
  class << self
    def load(path = File.join(DEFAULT_DIRECTORY_PATH, DEFAULT_DIRECTORY_NAME))
      ...
      if !exists
        raise RuntimeError.new("no repository found #{path}")
      end
